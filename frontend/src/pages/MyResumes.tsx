...
export const MyResumes: React.FC = () => {
  ...
  return (
		<>
			...
			>
				<CreateResumeModal
					onCloseModal={() => setIsOpen(!isOpen)}
					onCreateResume={refreshResumes}
				/>
			</ReactModal>
			...
			<div className="text-black z-1 absolute w-[90%] h-[100%] top-0 right-0 grid place-items-center ">
				<div className="w-[80%] h-[50%] group z-10 relative rounded-md shadow-xl bg-[#ffffff]">
					<strong className="font-bold p-5 text-2xl text-left text-gray-500 absolute top-[-15%] left-0">
						My Resumes
					</strong>
					<hr className="bg-[#F1A300] w-[20%] h-1 group-hover:w-[100%] transition-all duration-500" />

					<button className="absolute right-0 z-20 p-2 m-4 bg-[#F1A300] rounded hover:bg-[#F1A300]/80">
					...
					<div className="absolute w-[100%] h-[100%] grid place-items-center">
						<div
							className={`w-[90%] h-[60%] bg-[#F1A300]/25 place-self-center grid-rows-${resumeRows} rounded-md overflow-y-scroll p-5`}
						>
							<table className="bg-[#F1A300] text-left relative table-fixed w-[100%]">
								<tbody>
									{resumes.map((resume) => (
										<>
											<ResumeElement
