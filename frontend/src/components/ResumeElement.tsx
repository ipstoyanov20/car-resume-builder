...
function ResumeElement({resume, onResumeDelete}:{resume: ResumeVM, onResumeDelete: () => Promise<void>}) {
    const lastModified = resume.lastModified ? new Date(resume.lastModified) : undefined;
    const offset = lastModified?.getTimezoneOffset() ?? 0;
    const correctedLastModified = new Date((lastModified?.getTime() ?? 0) - offset * 60 * 1000);
    const relativeTime = new RelativeTime().from(new Date(correctedLastModified.toString()));
